...
class Tab {
  ...
  constructor({info, id, $task, $popover, match })
  {
    console.log($('.right-panel').width())
    this.info = info;
    this.id = id;
    this.$task = $('.task-container');
    this.$row = $task.find('.match-label').eq(this.id);
    this.$popover = $popover;
    this.match = match;
    this.$el = $('<div class="match-tab tab' + id + '"></div>');
    this.$line = $('<svg class="line line' + id
