export default class Timer {
  ...
  constructor(startTime, alertTime)
  {
  ...
  formatTime( opt )
  {
    let tHours = Math.floor(this.total / 3600000),
    ...
    switch( opt )
    {
      ...
      case 'l' :
        let lJSON = store.langData[store.currentLang].timer, pFx = lJSON.pluralPostfix;
  ...
  tick()
  {
    ...
    {
      if (this.total === 300000) { this.fiveMinuteWarning(); }
      else if (this.total < 300000) { this.timeString = this.formatTime( 's' ); }
      else { this.timeString = this.formatTime( 'm' ); }
