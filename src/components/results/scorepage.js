export default class Scorepage
{
  grade()
  {
    for( let theme of store.themes ) {
      for( let task of theme.tasks ) {
        ...
        task.correct = task.correct.reduce( (prev, curr) => prev && curr, true);
  ...
  buildAnswerKey()
  {
    let $keyWrapper = $('<div class="col-xs-12"></div>'),
    ...
    for (let theme of store.themes)
    {
      let $newThemeBody = $('<div class="key-theme-body"><div class="row"></div></div>');
      ...
      for (let task of theme.tasks)
      {
        $newThemeBody.children().eq(0).append( $keyWrapper.clone().append($(
          ...
          + `<p><span class="second-label">Question:</span>${task.question}</p></div>`)));
        ...
        let getIcon = check =>
            ...
            fillCell = ($parent, answer, key) =>
                {
                  $parent.append('<p>'
                    + (!key && answer.markWrong ? '<span class="fa fa-close"></span>' : '')
                    + (!key && answer.markRight ? '<span class="fa fa-check"></span>' : '')
                    + (task.type < 3
                        ? `<span class="fa ${getIcon((key ? answer.isCorrect : answer.isSelected))}"></span>${answer.text}</p>`
                        : `<span class="match-result">${answer.text}</span><br />`
                          + `<span class="match-result">${(key ? answer.isCorrect : answer.isSelected[0])}</span></p>`));
                },
        ...
        for (let answer in task.answers) { fillCell( $correct, answer, true); }
        
        if ( task.isCompleted )
        {
          for (let answer in task.answers) { fillCell( $answer, answer, false); }
        }
        else { $answer.append(
          $('<p class="incomplete"><span class="fa fa-close"></span><span class="incomplete-text">'
            + 'This question was left incomplete</span></p>'));
        }
        $answerContainer.append( $('<div class="row"></div>').append( $answer, $correct ));
        $newThemeBody.children().eq(0).append( $keyWrapper.clone().append( $answerContainer ));
      }
  ...
  //***   GET TOTAL FILL VALUE   ***//
    ...
    $('.total-bar path:nth-child(2)').css( {'stroke-dasharray': `${len}, ${len}`, 'stroke-dashoffset': len} );
  ...
  //***   ANIMATE BARS   ***//
      ...
      $(this).velocity({ 'stroke-dashoffset': (100 - themeFills[i] * 100) },
        {
          ...
          progress: trackProgress.bind( null, $('.theme-scores .percent-completed').eq(i), themeFills[i] )
      });
    });
    ...
    $('.total-bar .svg-progress-bar path:nth-child(2)')
      .velocity({ 'stroke-dashoffset': totalFill },
      {
        ...
        progress: trackProgress.bind( null, $('.total-bar .percent-completed'), totalFactor )
      });
  }

  init(updateLanguage, animateClick)
  {
    const self = this;
    ...
    /**********************
        RESET TEST
    **********************/
    $('#retryModal').on('hidden.bs.modal', () => { if (store.reset) { self.init(); } });
    
    $('.btn-modal-retry').click(() =>
    {
      store.themes = null;
      store.stats = null;
